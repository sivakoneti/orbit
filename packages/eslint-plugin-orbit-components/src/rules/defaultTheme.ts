...
export const ERRORS = {
  ...
  styled: "Do not use defaultTheme directly inside styled components, create defautlProps",
  import: "Do not name defaultTheme import as theme, please rename it to defaultTheme/themeDefault",
  desctructured: "Do not destructure defaultTheme, use useTheme() hook or ThemeConsumer",
};
      ...
      VariableDeclaration(node: t.VariableDeclaration) {
        node.declarations.forEach(n => {
          if (t.isVariableDeclarator(n)) {
            ...
            /**
              Prevents destruture of defaultTheme:
                const {
                  borderWidthCard,
                  borderStyleCard,
                  borderColorCard,
